#!/usr/bin/env node
// triad-all — Layer 2 self-check (copy of triad-codex template, SELF_NAME changed).
//
// Proves this skill is self-contained before runtime or distribution.
// Fails if any file references another plugin/skill, declares a _shared/
// dependency, or is missing a required internal file.
...
// triad-all requires both Claude Code (ambient — cannot probe) and Codex CLI.
// Claude Code presence is assumed (the skill can't run without it). Only Codex
// is probed because it's an external CLI dependency.
const PREREQUISITES = [
  ...
  for (const p of PREREQUISITES) {
    try {
      ...
      if (p.min) {
        ...
        if (match && compareSemver(match[1], p.min) < 0) {
          const msg = `prerequisite version too old: ${p.label} ${match[1]} < required ${p.min}`;
